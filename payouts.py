...
class Payout:
    def __init__(self,root):
        ...
        m_title=Label(Manage_Frame, text="Manage Payouts",bg="crimson",fg="white", font=("monospace",30,"bold"))
        ...
        Addbtn=Button(btn_Frame,command=self.add_employee,text="Issue",width=5).grid(row=0,column=0,padx=8,pady=10)
        updatebtn=Button(btn_Frame,command=self.update_data,text="Delete",width=7).grid(row=0,column=1,padx=8,pady=10)
    ...
    def get_employees(self):
        ...
        self.employees.set("")
        self.color_option_field = OptionMenu(Manage_Frame, self.employees, *dict_options.values(), command=self.fetch_employee)
        self.color_option_field.grid(row=1,column=1)
        self.color_option_field['width'] = 10
        con.close()

    def fetch_employee(self, value):
        global dict_options
        if self.Stock_Table.get_children():
            self.Stock_Table.delete(*self.Stock_Table.get_children())
        key_list = list(dict_options.keys())
        val_list = list(dict_options.values())
        position = val_list.index(value+1)
